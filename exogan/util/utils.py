...
def planckel(wavelength, temperature=5800):
    ...
    """
    return 3.7418301e8 / (
        wavelength**5 * (np.exp(14387.86e0 / (wavelength * temperature)) - 1.0)
    )
...
def binary_cross_entropy(preds, targets, name=None):
    ...
    with ops.op_scope([preds, targets], name, "bce_loss") as name:
        ...
        return tf.reduce_mean(
            -(
                targets * tf.log(preds + eps)
                + (1.0 - targets) * tf.log(1.0 - preds + eps)
            )
        )
...
def conv_cond_concat(x, y):
    ...
    return tf.concat(
        3, [x, y * tf.ones([x_shapes[0], x_shapes[1], x_shapes[2], y_shapes[3]])]
    )
...
def get_spectral_matrix(path, parfile=None, size=23):
    ...
    norm_bounds = np.array(
        [0.63, 0.77, 0.88, 1.0, 1.1, 1.25, 1.8, 2.13, 3.96, 4.56, 6.33, 7.21, 10.0]
    )
    ...
    if type(path) == str and path[-3:] == "dat":
        ...
        wnw_min = find_nearest(wnw_grid, max(spec[:, 0]))
        wnw_max = find_nearest(wnw_grid, min(spec[:, 0]))
        ...
        atm_active_gases = np.array(
            [gas.upper() for gas in real_pars["Atmosphere"]["active_gases"]]
        )
    ...
    for i in range(len(norm_idx) - 1):
        ...
        if minf == maxf:
            norm_spectrum[norm_idx[i] : norm_idx[i + 1]] = 0.0
        elif i == len(norm_idx) - 2:
            norm_spectrum[norm_idx[i] : norm_idx[i + 1]][:-4] = (frag - minf) / (
                maxf - minf
            )
...
def make_corner_plot(all_hists, ranges, labels, ground_truths, comppars, index):
    ...
    histName = os.path.join(
        comppars["outDir"], "histograms/corner/{:04d}.pdf".format(index)
    )
...
def recon_spectrum(spec, size=23):
    ...
    for i in range(len(norm_idx) - 1):
        ...
        if in_spec[norm_idx[i]] == in_spec[int(const_control)]:
            in_spec[norm_idx[i] : norm_idx[i + 1]] = 0.0
        elif i == len(norm_idx) - 2:
...
def spectra_int_norm(Xtrue, imgsz, wnw_grid, batchSz, G_imgs, comppars, i):
    ...
    for k in range(batchSz):
        ...
        chi_square.append(
            (
                ((spectrum[: len(wnw_grid) - 4] - real_spec[: len(wnw_grid) - 4]) ** 2)
                / real_spec[: len(wnw_grid) - 4]
            )
            .sum()
            .sum()
            # chisquare(
    ...
    ax.plot(
        wnw_grid[:-4],
        spectra[best_ind][: len(wnw_grid) - 4],
        "r-",
    ...
    histName = os.path.join(
        comppars["outDir"], "spectra/inbuilt/best/{:04d}.pdf".format(i)
    )
    plt.xscale("log")
    plt.xticks([1, 10], ["1", "10"])
    #          ax.set_ylim(global_minimum, global_maximum)
...
def spectra_norm(Xtrue, imgsz, wnw_grid, batchSz, G_imgs, comppars, i):
    ...
    for k in range(batchSz):
        ...
        spectra.append(spectrum)
        chi_square.append(
            (((spectrum[:440] - real_spec[:440]) ** 2) / real_spec[:440])
            .sum()
            .sum()
    ...
    histName = os.path.join(
        comppars["outDir"], "spectra/normalised/best/{:04d}.pdf".format(i)
    )
    plt.xscale("log")
    plt.xticks([1, 10], ["1", "10"])
    #          ax.set_ylim(global_minimum, global_maximum)
...
def spectra_real_norm(Xtrue, imgsz, wnw_grid, batchSz, G_imgs, comppars, i):
    ...
    for k in range(batchSz):
        ...
        spectra.append(spectrum)
        chi_square.append(
            (((spectrum[:-8] - real_spec[:-8]) ** 2) / real_spec[:-8])
            .sum()
            .sum()
    ...
    ax.plot(
        wnw_grid[:-8],
        spectra[best_ind][: len(wnw_grid) - 8],
        "r-",
        label="generated spectrum",
    )
    ax.plot(wnw_grid[:-8], real_spec[: len(wnw_grid) - 8], "-k", label="real spectrum")
    ...
    histName = os.path.join(
        comppars["outDir"], "spectra/with_real_norm/best/{:04d}.pdf".format(i)
    )
    plt.xscale("log")
    plt.xticks([1, 10], ["1", "10"])
